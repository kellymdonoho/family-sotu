...
import { Heart, Bell, BellOff, Mail } from "lucide-react";

// ── ROLE MAP ──────────────────────────────────────────────────────────────────
// Each authorized user maps to a role:
//   "parent"  — full access to all tabs (Kelly & Kevin)
//   "calendar" — read-only access to the calendar/logistics tab only
//
// To invite a new person, add their email here and redeploy.
// Mom (Yahoo email) uses email/password sign-in — see SETUP.md for Firebase config.
const USER_ROLES = {
...
export default function App() {
  ...
  // Email/password sign-in form state
  const [signInMode, setSignInMode]   = useState("google"); // "google" | "email"
      ...
      if (u) {
        ...
        if (role === "parent") {
          setDoc(doc(db, "meta", "roles"), USER_ROLES, { merge: true })
            .catch(e => console.error("Could not sync roles:", e));
        }
      } else {
        setUserRole(null);
      }
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      if (!role) {
        await signOut(auth);
        setError("This Google account is not authorized. Ask Kelly or Kevin to add you.");
      }
    } catch (e) {
      setError(e.message);
    }
  ...
  const signInWithEmail = async (isSignUp) => {
    setError(null);
    ...
    try {
      ...
      if (!role) {
        await signOut(auth);
        setError("This email is not authorized. Ask Kelly or Kevin to add you.");
      }
    } catch (e) {
      setError(e.message);
    }
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-8 shadow-sm max-w-sm w-full text-center">
        ...
        {signInMode === "google" ? (
          <>
            ...
            <p className="text-xs text-stone-400 mt-4">Kelly and Kevin sign in with Google</p>
            <button onClick={() => { setError(null); setSignInMode("email"); }}
              className="mt-3 text-xs text-stone-500 hover:text-slate-700 font-semibold underline underline-offset-2">
          ...
          <>
            <div className="space-y-3 text-left">
              ...
              <input type="email" value={emailForm.email} onChange={e=>setEmailForm(f=>({...f,email:e.target.value}))}
                placeholder="your@email.com"
                className="w-full text-sm border border-stone-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-slate-400"/>
              <input type="password" value={emailForm.password} onChange={e=>setEmailForm(f=>({...f,password:e.target.value}))}
                placeholder="Password"
                className="w-full text-sm border border-stone-200 rounded-xl px-3 py-2.5 focus:outline-none focus:border-slate-400"/>
            </div>
            <button onClick={()=>signInWithEmail(false)} disabled={emailSigningIn}
              className="w-full mt-4 py-3 bg-slate-900 text-white text-sm font-bold rounded-xl hover:bg-slate-800 disabled:opacity-40 transition-colors">
              {emailSigningIn ? "Signing in..." : "Sign in"}
            </button>
            <button onClick={()=>signInWithEmail(true)} disabled={emailSigningIn}
              className="w-full mt-2 py-2.5 bg-white border border-stone-300 text-slate-700 text-sm font-semibold rounded-xl hover:bg-stone-50 disabled:opacity-40 transition-colors">
              Create account
            </button>
            <button onClick={() => { setError(null); setSignInMode("google"); setEmailForm({email:"",password:""}); }}
              className="mt-3 text-xs text-stone-500 hover:text-slate-700 font-semibold underline underline-offset-2">
  ...
  // Authorized user — render the app with their role
  if (!userRole) {
    // Signed in but not in USER_ROLES (shouldn't happen — we sign them out — but guard anyway)
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center px-4">
        <div className="bg-white border border-stone-200 rounded-2xl p-8 shadow-sm max-w-sm w-full text-center">
          <Heart className="w-10 h-10 text-rose-400 mx-auto mb-4"/>
          <p className="text-sm text-stone-600 mb-4">This account is not authorized.</p>
          <button onClick={()=>signOut(auth)} className="text-sm font-semibold text-slate-700 underline">Sign out</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <FamilySOUnion db={db} user={user} role={userRole} onSignOut={()=>signOut(auth)}/>
    </div>
  );
}
